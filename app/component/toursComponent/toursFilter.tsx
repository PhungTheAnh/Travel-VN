...
import { placesFilter, prices } from '@/app/component/data/data';
...
const ToursFilter = () => {
  ...
  const TourResult1 = (props: { tour: any; idx: number }) => {
    ...
    return (
      ...
      >
        <div className='relative h-[250px] w-full rounded-lg overflow-hidden shadow-md'>
          ...
          />
          <div className='absolute bottom-0 right-0 p-2.5 rounded-tl-[12px] bg-white flex items-center gap-[10px]'>
        ...
        <div className='flex flex-col gap-[10px] w-full'>
          <Link
            href='#'
            className='text-lg leading-[1.35rem] w-full text-[#252525] hover:text-primary text-left font-bold line-clamp-2'
          >
            {tour.name}
          </Link>
          ...
          <div className='flex items-center justify-between font-medium'>
            <div className='text-sm text-primary'>
              <p>Giá</p>
              <p className='text-lg leading-[1.5rem] md:leading-[1.75rem] font-bold'>
                {VND.format(tour.cost)}
              </p>
            </div>
            <div>
              <div className='flex gap-0.5 items-center'>{htmlStars}</div>
              <p className='text-darkCharcoal text-xs font-normal'>
                ({tour.comments} đánh giá)
              </p>
            </div>
          </div>
          <Link
            href={`tours/${tour.name}`}
            className='btnOrder1 text-white text-base w-full mx-auto transition-all duration-300 flex justify-center items-center bg-[#4366c5] rounded-lg p-[15px]'
          >
            Đặt ngay
          </Link>
        </div>
  ...
  const TourResult2 = (props: { tour: any; idx: number }) => {
    const { tour, idx } = props;
    let htmlStars = [];
    for (let i = 0; i < tour.star; i++) {
      htmlStars.push(<AiFillStar key={i} size={14} color={'#ffa11a'} />);
      ...
      >
        ...
        <div className='w-full md:w-[20%]'>
          <div className='flex md:flex-col justify-between items-center gap-[15px] font-medium'>
            ...
            <Link
              href='#'
              className='btnOrder2 w-0 text-white text-sm flex justify-center items-center bg-[#4366c5] rounded-lg p-[15px] transition-all duration-300'
            >
              Đặt ngay
            </Link>
          </div>
  ...
  return (
    <Tab.Group>
      <div className='w-full 2xl:w-[85%] mx-auto relative flex flex-col lg:flex-row items-start justify-center gap-[30px]'>
        <div
          ref={menuFilter}
        ...
        >
          <p className='text-2xl text-darkCharcoal py-[15px] font-semibold'>
            Lọc thông tin
          </p>
        ...
        <div className='relative w-full lg:w-3/4'>
          <div>
            <Tab.Panels>
              {Object.values(placesFilter).map((result, index) => {
                return (
                  <Tab.Panel key={index}>
                    <div className='flex justify-between items-center w-full border-b border-[#a6c3ea] pb-[15px] mb-[30px]'>
                      <div className='flex items-center gap-2'>
                        ...
                        <p className='text-[30px] font-bold text-left text-black'>
                          Tour du lịch
                        </p>
                      </div>
                      <div
                    ...
                    >
                      {result.map((tour: any, idx) => {
                        return <TourResult1 key={idx} tour={tour} idx={idx} />;
                      })}
                    </div>
                    <div
                      ref={toursResult2}
                    ...
                    >
                      {result.map((tour: any, idx) => {
                        ...
                        for (let i = 0; i < tour.star; i++) {
                          htmlStars.push(
                            <AiFillStar key={i} size={14} color={'#ffa11a'} />
                          );
                        }
                        return (
                          <div
